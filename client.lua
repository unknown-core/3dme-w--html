...
--             SetTextScale(0.0*scale, scale)
	 SetTextScale(0.90*scale, 0.90*scale)
--         else 
...
Citizen.CreateThread(function()
    ...
    while true do
        for _, id in ipairs(GetActivePlayers()) do
		    ...
		    local iPed = GetPlayerPed(id)
		    local lPed = PlayerPedId()
                    if playerDistances[id] < disPlayerNames and HasEntityClearLosToEntity(lPed, iPed, 17) then
                        local targetPedCords = GetEntityCoords(targetPed)

                        if NetworkIsPlayerTalking(id) then
                            DrawText3D(targetPedCords, GetPlayerServerId(id),50,205,50)
...
-- 			    print(HasEntityClearLosToEntity(lPed, iPed, 17))
